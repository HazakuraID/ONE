...
const outputTemplate = "Rilis ini berisi semua skrip yang disediakan oleh repositori ini.\n\nSilahkan lihat [halaman proyek utama](https://github.com/{{.repo}}) untuk deskripsi tentang scripts.{{if .stats}}\n\n**Stats**:{{range .stats}}\n* `{{.ScriptName}}`: {{.StatsLine}}{{end}}{{end}}"
...
func main() {
	var (
		...
		repo = env("GITHUB_REPOSITORY", "HazakuraID/ONE")
